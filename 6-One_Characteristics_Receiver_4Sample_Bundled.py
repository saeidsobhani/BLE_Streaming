...
async def main():
    ...
    def on_combined(sender, data: bytearray):
        ...
        # Each sample is 12 bytes (6 × int16)
        # Payload can contain 1-4 samples bundled together
        data_len = len(data)
        num_samples = data_len // 12  # Calculate how many samples in this notification
        
        if data_len % 12 != 0:
            print(f"Warning: Invalid data length {data_len}, expected multiple of 12")
            return
        
        # Unpack all samples in this notification
        for i in range(num_samples):
            offset = i * 12  # Each sample is 12 bytes
            sample_data = data[offset:offset+12]
            
            # Unpack: [ax, ay, az, mx, my, mz] as int16 LE
            ax_mg, ay_mg, az_mg, mx_raw, my_raw, mz_raw = struct.unpack("<hhhhhh", sample_data)
            ...
            # print(f"Sample {i+1}/{num_samples}: Accel(g): {accel_g}, Mag(µT): {mag_uT}")
    ...
    async def print_rates():
        ...
        while True:
            ...
            print(f"Sample rate: {sample_hz:.1f} Hz (Accel & Mag)")
    ...
    async with BleakClient(target) as client:
        ...
        USE_MTU_NEGOTIATION = True  # Set to False to use default MTU = 23
        ...
        max_samples_per_notify = (mtu - 3) // 12
        ...
        try:
            test_value = await client.read_gatt_char(COMBINED_CHAR_UUID)
        ...
        try:
            print(f"Attempting to subscribe to characteristic: {COMBINED_CHAR_UUID}")
            await client.start_notify(COMBINED_CHAR_UUID, on_combined)
            print("✓ Successfully subscribed to combined accel+mag notifications.")
            print("✓ Callback function registered: on_combined")
            print("✓ Waiting for notifications from Bangle.js...")
        except Exception as e:
            print(f"❌ Failed to subscribe to combined characteristic: {e}")
            import traceback
            traceback.print_exc()
            return
