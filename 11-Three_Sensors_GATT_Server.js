...
Bangle.setHRMPower(1); // Enable HRM sensor
Bangle.compassWr(0x31, 0x08); // Configure magnetometer ODR to 100 Hz
Bangle.setPollInterval(10); // Configure sensor poll interval to 100 Hz

// Configure accelerometer ODR to 400 Hz and enable Stream with 8-bit mode MSB
var i2c = new I2C();
i2c.setup({scl:14, sda:15});
i2c.writeTo(0x1E, [0x18, 0b00101100]); // Enter standby mode - output range +-4g
i2c.writeTo(0x1E, [0x1B, 0x05]);       // Set ODCNTL to have 400 Hz ODR
...
NRF.setServices({
  'f26d62fe-3686-4241-ab06-0dad88068fac': {
    ...
    'f26d62fe-3686-4241-ab06-0dad88068fbf': {
      description: 'HRM Raw',
      notify: true,
      readable: true,
      value: new Int8Array(1).buffer
    }
  }
}, { uart: true });
